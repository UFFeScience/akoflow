...
2. **P0 — Generated request contracts:** ZIP instance import is corrected and inferred JSON is labeled. Six first-run POST pages link the verified, versioned SimGrid payloads and their submission order. Twenty more pages carry handler/service-checked notes for credentials, cloud provisioning, planning, storage, and artifact operations. The planning-session page uses a concise request example with the actual required fields. Other priority endpoints still need handler-checked required fields and runnable payloads.

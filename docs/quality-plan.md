...
5. Generated endpoint pages identify inferred JSON shapes, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to all 55 other mutating routes. The generator now rejects uncovered mutations. The shared 125-page template no longer repeats each field or exposes Go type names; its struct index preserves the console request and resolves qualified environment/workflow responses. The next P0 pass must validate inferred JSON examples field by field against handlers and actual submissions; notes and build success do not prove that every copied request works.

...
- [x] All internal links and every downloadable asset pass an automated link check. Evidence: `docs/scripts/check-links.mjs` and `.github/workflows/docs-checks.yaml`; the latest check covered 396 local route/asset links and 54 Showcase downloads on 2026-09-13.
- [x] Fenced Bash/sh examples pass a syntax check in CI. Evidence: `docs/scripts/check-shell-examples.mjs` checked 109 authored blocks on 2026-09-13; syntax does not verify files, API requests, or JSX-embedded commands.
...
5. Generated endpoint pages identify inferred JSON shapes, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to thirty-nine more routes, including instance/environment updates, connection health/discovery, workflow actions, cloud catalog/capacity, planning cancellation and candidate selection, storage, artifacts, and console. The shared 125-page template no longer repeats each field or exposes Go type names; its struct index preserves the console request and resolves qualified environment/workflow responses. Remaining priority request contracts still need required fields, valid examples, and handler checks. Build and link validation do not prove that a copied request works.

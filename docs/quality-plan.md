...
5. Generated endpoint pages identify inferred JSON shapes, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to twenty-seven more routes, including storage, artifacts, console, connection tests, and machine configuration. The shared 125-page template no longer repeats each field or exposes Go type names, and its struct index no longer substitutes the SimGrid request for a console command. Remaining priority request contracts still need required fields, valid examples, and handler checks. Build and link validation do not prove that a copied request works.

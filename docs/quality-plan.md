...
3. Additional focused Desktop captures would improve cross-links between provenance and individual run detail. The planning, environment-catalog, execution-scope, workflow-history and SimGrid run-detail captures are reviewed in light and dark themes; the automated link checker is in CI.

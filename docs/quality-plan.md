...
- [ ] Add one complete SLURM showcase only when a safe test cluster or reproducible local SLURM fixture is available. In progress: `examples/slurm/local-fixture` supplies the isolated fixture; retain this marker until its repository bundle completes a real local adapter run and the Showcase passes documentation checks.

...
- [x] All internal links and every downloadable asset pass an automated link check. Evidence: `docs/scripts/check-links.mjs` and `.github/workflows/docs-checks.yaml`; the latest check covered 401 local route/asset links and 54 Showcase downloads on 2026-09-13.
- [x] Fenced Bash/sh examples pass a syntax check in CI. Evidence: `docs/scripts/check-shell-examples.mjs` checked 110 authored blocks on 2026-09-13; syntax does not verify files, API requests, or JSX-embedded commands.
...
5. Generated endpoint pages label inferred JSON as field shapes at the point of use, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to all 55 other mutating routes. The generator now rejects uncovered mutations. The shared 125-page template calls incomplete cURL commands templates and no longer repeats a page-end caveat or exposes Go type names; its struct index preserves the console request and resolves qualified environment/workflow responses. The two provenance SQL POST routes have request bodies decoded by handler tests and executed/explained by repository tests. Machine-configuration validation has an exact JSON request that returns a valid digest; configuration creation and versioning have exact JSON requests persisted through a bootstrapped SQLite handler test. The build-context page shows the handler-tested multipart upload path instead of a metadata-only JSON template. Docker artifact registration uses the same request as the user guide and has a handler/SQLite test proving saved version and build specification, not a completed image build. Storage promotion now has explicit lower-camel handler tags and guide-matching request examples checked by handler tests; file/path preconditions still apply. The remaining P0 pass must validate other inferred JSON examples field by field against handlers and actual submissions; notes and build success do not prove that every copied request works.

...
5. Generated endpoint pages identify inferred JSON shapes, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to thirty-eight more routes, including instance/environment updates, connection health/discovery, workflow actions, cloud catalog/capacity, candidate selection, storage, artifacts, and console. The shared 125-page template no longer repeats each field or exposes Go type names; its struct index preserves the console request and resolves qualified environment/workflow responses. Remaining priority request contracts still need required fields, valid examples, and handler checks. Build and link validation do not prove that a copied request works.

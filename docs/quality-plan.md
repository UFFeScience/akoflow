...
3. Additional focused Desktop captures would improve the SQL interaction example. The planning, environment-catalog, execution-scope, workflow-history, SimGrid run-detail, provenance-explore, and provenance-lineage captures are reviewed in light and dark themes; the automated link checker is in CI.

...
- [x] All internal links and every downloadable asset pass an automated link check. Evidence: `docs/scripts/check-links.mjs` and `.github/workflows/docs-checks.yaml`; the latest check covered 394 local route/asset links and 54 Showcase downloads on 2026-09-13.
...
5. Generated endpoint pages identify inferred JSON shapes, handle ZIP instance import, link six runnable SimGrid POST payloads, and add handler-checked notes to thirty more routes, including instance/environment updates, storage, artifacts, console, connection tests, and machine configuration. The shared 125-page template no longer repeats each field or exposes Go type names; its struct index preserves the console request and resolves qualified environment/workflow responses. Remaining priority request contracts still need required fields, valid examples, and handler checks. Build and link validation do not prove that a copied request works.

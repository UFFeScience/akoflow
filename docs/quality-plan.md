...
- [x] All internal links and every downloadable asset pass an automated link check. Evidence: `docs/scripts/check-links.mjs` and `.github/workflows/docs-checks.yaml`; the latest check covered 458 local route/asset links and 54 Showcase downloads on 2026-09-13.
...
- [x] Generate or maintain a field-level environment YAML reference from the authoritative domain types, including required fields, defaults, enum values, and compatibility. The 2026-09-13 audit corrected API input defaults that SQLite schema defaults had obscured; the local environment example persisted in a one-off repository test.
- [x] Do the same for execution scopes and network topologies. The 2026-09-13 audit corrected omitted link-direction and sharing values against the topology repository and SimGrid mapper.
...
- [x] Replace active structural ASCII diagrams with accessible versioned SVGs. Evidence: `static/img/architecture/` contains 13 lifecycle, control-plane, planning, network, scope, evidence, timing, interface-hierarchy, and troubleshooting diagrams; each parses with a `<title>` and `<desc>` and its page supplies alternative text. Arrow paths no longer inherit fill that obscured branch lines; four representative diagrams were previewed in Chromium on 2026-09-13.
...
5. The 125 generated endpoint pages no longer display inferred request bodies; all 61 mutating routes have handler-checked notes or a versioned SimGrid request. Concrete requests have tests for provenance SQL, machine configuration, Docker artifact registration, storage operations, console field decoding, SSH key generation, a local connection probe, instance preferences, workflow import/duplication, and inventory-only resource upsert. The build-context page shows multipart upload; the instance and environment replacement pages direct readers to modify their current records. Artifact build and materialization registration pages state their prerequisites; materialization's `environmentId` must identify an environment version. Credential and provider routes require real user inputs. An isolated local-storage HTTP run now covers browse, download, checksum, copy, and archive retrieval; it exposed and verified the fix for an archive-path persistence defect. Remote and provider storage remain unverified. Schedule-plan import now has a saved-plan copy example with unique plan, assignment, and lifecycle action IDs. An isolated HTTP test returned `201` for import and `200` for the copied plan GET, with dependencies and predictions preserved. Seven delegated cloud-operation pages show `202 Accepted`; the console WebSocket stream shows `101 Switching Protocols` and a WebSocket URL instead of a cURL template. The generator fails on unknown success statuses instead of silently displaying `200 OK`; illustrative JSON response shapes are labeled before the object. The cloud target example now has source-checked `configuration.projectId` and built-in-worker-compatible `amd64`, but no live provider submission. Plan creation/import notes now correctly state that supplied predicted metrics are saved without recalculation. **P0 still open:** full field-level request and response validation across all endpoints, particularly cloud provisioning. Handler-note coverage and a green build do not prove copied requests work or provider operations succeed.

Schedule import HTTP check: the first isolated submission on 2026-09-13 returned `422` because the saved SimGrid plan has four generated lifecycle actions whose IDs also need changing. The corrected documented jq command passed against a fresh isolated server: the five prerequisite registrations returned `201`, import returned `201`, and copied-plan GET returned `200`. All three assignments, four lifecycle dependencies, and predicted metrics were verified.

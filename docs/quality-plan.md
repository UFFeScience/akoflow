...
5. The 125 generated endpoint pages label inferred bodies as field shapes; all 61 mutating routes have handler-checked notes or a versioned SimGrid request. Concrete requests have tests for provenance SQL, machine configuration, Docker artifact registration, storage operations, console field decoding, a local connection probe, instance preferences, workflow import/duplication, and inventory-only resource upsert. The build-context page shows multipart upload; the instance and environment replacement pages direct readers to modify their current records. Artifact build and materialization registration pages now state their prerequisites without speculative bodies; materialization's `environmentId` must identify an environment version. Storage paths and console resources remain user-specific, and registering a Docker build does not execute it. **P0 still open:** 9 inferred bodies remain displayed and need validation against handlers and actual submissions. Build and route-note coverage alone do not prove copied requests work.

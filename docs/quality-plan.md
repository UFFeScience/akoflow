...
- [x] All internal links and every downloadable asset pass an automated link check. Evidence: `docs/scripts/check-links.mjs` and `.github/workflows/docs-checks.yaml`; the latest check covered 406 local route/asset links and 54 Showcase downloads on 2026-09-13.
...
5. The 125 generated endpoint pages label inferred bodies as field shapes; all 61 mutating routes have handler-checked notes or a versioned SimGrid request. Concrete requests have tests for provenance SQL, machine configuration, Docker artifact registration, storage operations, console field decoding, SSH key generation, a local connection probe, instance preferences, workflow import/duplication, and inventory-only resource upsert. The build-context page shows multipart upload; the instance and environment replacement pages direct readers to modify their current records. Artifact build and materialization registration pages now state their prerequisites without speculative bodies; materialization's `environmentId` must identify an environment version. Credential import, token storage, and cloud validation pages require real user credentials instead of displaying fictitious secret bodies. Storage paths and console resources remain user-specific, and registering a Docker build does not execute it. **P0 still open:** 4 inferred bodies remain displayed and need validation against handlers and actual submissions. Build and route-note coverage alone do not prove copied requests work.

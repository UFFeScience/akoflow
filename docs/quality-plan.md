...
5. The 125 generated endpoint pages label inferred bodies as field shapes; all 61 mutating routes have handler-checked notes or a versioned SimGrid request. Concrete requests now have tests for provenance SQL, machine configuration, Docker artifact registration, storage operations, console field decoding, and a local connection probe. The build-context page shows multipart upload. Storage paths and console resources remain user-specific, and registering a Docker build does not execute it. **P0 still open:** validate the other inferred bodies against handlers and actual submissions. Build and route-note coverage alone do not prove copied requests work.

...
const verifiedRequestNotes = {
  ...
  "PUT /akoflow-api/environments/{environmentId}/": "Send a complete environment definition. `environment.id` must match the path ID. Replacement returns `404` when that environment does not exist and can return `422` when references prevent replacing its inventory. Use new environment and version IDs for revised inventory already used by scopes or plans; see the [environment YAML reference](/docs/reference/environment-yaml#compatibility-and-common-failures).",
  ...
  "PUT /akoflow-api/environment-connections/{connectionId}/": "Send a complete connection object: its `id` must match the path and `environmentId` must be non-empty. The repository inserts a new connection or replaces the saved connection fields, so read an existing connection before changing one setting. The response is the submitted object; this route does not run a health check.",
  ...
  "POST /akoflow-api/console-sessions/": "`resourceId` must identify a resource with a connected interactive runtime; `actorId` is optional. A successful request starts the terminal and returns a `connected` session. Resolution or startup failure returns `422`, not a successful session record. Use the [console guide](/docs/guides/operations/interactive-console) for streaming and closure.",
  "POST /akoflow-api/console-commands/": "`resourceId` and `command` are required. `timeoutSeconds` defaults to 30 and cannot exceed 3600. This call waits for the runner and returns a command record. A runner failure can return `201 Created` with `status: failed`; inspect `status`, `exitCode`, and `failure` instead of treating the HTTP status as command success.",

...
const verifiedRequestNotes = {
  ...
  "POST /akoflow-api/schedule-plans/import/": "Send a JSON object with a complete `plan`. Use a unique plan `id` and matching assignment `planId` values. Its workflow version, execution scope, topology, and resources must already exist. The server sets `source` to `imported`, checks the schedule against those saved records, and returns `422` if it is invalid. It saves the supplied predicted metrics without recalculating them. The response shape below shows the plan fields; a body filled with placeholder IDs would not pass validation.",

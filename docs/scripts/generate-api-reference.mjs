...
const verifiedRequestNotes = {
  ...
  "POST /akoflow-api/environments/{environmentId}/cloud-capacity-targets/": "Use an existing cloud environment with a cloud runtime. Send a target definition with `name`, `providerMachineType`, and `imageReference`; the server supplies an omitted `id`, sets `environmentId` from the path, and enables the target. Provider region, image, machine type, and network policy are account-specific; use the [GCP guide](/docs/guides/infrastructure/gcp) before creating one. Set `configuration.projectId` to the validated project: Terraform reads it from the target and does not copy it from the connection. The built-in worker configuration requires `amd64` target architecture. The route creates a schedulable capacity resource but no VM. Set approved `configuration.sshSourceRanges` before provisioning: the Terraform target otherwise defaults SSH ingress to `0.0.0.0/0`. If creation returns `422`, inspect the target list before retrying because target persistence precedes resource binding.",

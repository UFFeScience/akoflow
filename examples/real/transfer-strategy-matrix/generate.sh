...
while IFS="$(printf '\t')" read -r case_id source target; do
  ...
  produce_command=$(printf "set -eu\nprintf 'route=%s-%s\\nsequence=1\\n' > payload.txt\nsha256sum payload.txt > payload.sha256\ntest -s payload.txt" "$source" "$target")
  consume_command=$(printf "set -eu\ntest -s payload.txt\nsha256sum -c payload.sha256\ngrep -qx 'route=%s-%s' payload.txt\nprintf 'consumed=%s\\n' > receipt.txt\ntest -s receipt.txt" "$source" "$target" "$target")
  ...
  jq -n \
    ...
    --arg produceCommand "$produce_command" \
    --arg consumeCommand "$consume_command" \
    --argjson payloadSize "$payload_size" \
    '{name:$name,spec:{namespace:"transfer-matrix",image:"docker.io/library/busybox:1.36",activities:[
      {name:"produce",runtime:$sourceRuntime,cpuLimit:"0.1",memoryLimit:"16Mi",run:$produceCommand},
      {name:"consume",runtime:$targetRuntime,cpuLimit:"0.1",memoryLimit:"16Mi",dependsOn:["produce"],run:$consumeCommand}
    ],dataDependencies:[
  ...
  jq -n \
    ...
    '{plan:{id:$id,workflowVersionId:$workflowVersionId,executionScopeId:"transfer-matrix-scope-v2",networkTopologyId:"transfer-matrix-network-v2",source:"imported",algorithm:"manual",algorithmVersion:"2",objective:"transfer-validation",predicted:{makespanSeconds:300,cost:0.01,feasible:true},assignments:[

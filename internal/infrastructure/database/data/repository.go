...
func (r *Repository) SaveTransferRun(ctx context.Context, value domain.DataTransferRun) error {
	...
	_, err = r.db.ExecContext(ctx, `INSERT INTO transfer_runs(id,plan_id,execution_run_id,activity_id,strategy,status,verified_blobs,completed_chunks,started_at,finished_at,transferred_bytes,error,logical_bytes,network_bytes,route)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET execution_run_id=excluded.execution_run_id,
		activity_id=excluded.activity_id, strategy=excluded.strategy, status=excluded.status, verified_blobs=excluded.verified_blobs,
		...
		network_bytes=excluded.network_bytes, route=excluded.route`, value.ID, value.PlanID,
		value.ExecutionRunID, value.ActivityID, value.Strategy, value.Status, string(verified), string(chunks),
		value.StartedAt, value.FinishedAt, value.TransferredBytes, value.Error, value.LogicalBytes,
		value.NetworkBytes, string(route))
	return err
}

func (r *Repository) ListArtifactTransferRuns(ctx context.Context, runID string) ([]domain.DataTransferRun, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id,plan_id,execution_run_id,activity_id,strategy,status,verified_blobs,completed_chunks,started_at,finished_at,transferred_bytes,error,logical_bytes,network_bytes,route
		FROM transfer_runs WHERE execution_run_id=? OR (execution_run_id='' AND id IN (
			SELECT 'transfer-' || id FROM artifact_materializations WHERE run_id=?
	...
	for rows.Next() {
		...
		if err := rows.Scan(&value.ID, &value.PlanID, &value.ExecutionRunID, &value.ActivityID,
			&value.Strategy, &value.Status, &verified, &chunks, &value.StartedAt, &value.FinishedAt,
			&value.TransferredBytes, &value.Error, &value.LogicalBytes, &value.NetworkBytes, &route); err != nil {
			return nil, err
		}
...
func (r *Repository) FindTransferRun(ctx context.Context, id string) (*domain.DataTransferRun, error) {
	var value domain.DataTransferRun
	var verified, chunks, route string
	err := r.db.QueryRowContext(ctx, `SELECT id,plan_id,execution_run_id,activity_id,strategy,status,verified_blobs,completed_chunks,started_at,finished_at,transferred_bytes,error,logical_bytes,network_bytes,route FROM transfer_runs WHERE id=?`, id).
		Scan(&value.ID, &value.PlanID, &value.ExecutionRunID, &value.ActivityID, &value.Strategy,
			&value.Status, &verified, &chunks, &value.StartedAt, &value.FinishedAt,
			&value.TransferredBytes, &value.Error, &value.LogicalBytes, &value.NetworkBytes, &route)

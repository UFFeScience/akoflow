...
CREATE TABLE schedule_plans (
		...
		source TEXT NOT NULL CHECK(source IN ('plugin', 'manual', 'imported')),
...
CREATE TABLE planning_sessions (
		...
		selected_plan_id TEXT REFERENCES schedule_plans(id),
		deadline_seconds REAL NOT NULL DEFAULT 0,
		budget REAL NOT NULL DEFAULT 0,
		configuration TEXT NOT NULL DEFAULT '{}',
...
CREATE TABLE planning_algorithm_runs (
		...
		started_at DATETIME,
		completed_at DATETIME,
		UNIQUE(planning_session_id, algorithm)
	);
CREATE TABLE planning_candidates (
		...
		created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
		UNIQUE(planning_session_id, fingerprint)
	);

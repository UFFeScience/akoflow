...
func (w *ActivityRepository) Find(id int) (workflow_activity_entity.WorkflowActivities, error) {
	...
	rows, err := c.Query("SELECT id, workflow_id, namespace, name, image, runtime, resource_k8s_base64, status, proc_id, node_selector FROM "+w.tableNameActivity+" WHERE id = ?", id)
	if err != nil {
	...
	for rows.Next() {
		err = rows.Scan(&wfaDatabase.Id, &wfaDatabase.WorkflowId, &wfaDatabase.Namespace, &wfaDatabase.Name, &wfaDatabase.Image, &wfaDatabase.Runtime, &wfaDatabase.ResourceK8sBase64, &wfaDatabase.Status, &wfaDatabase.ProcId, &wfaDatabase.NodeSelector)
		if err != nil {
			return workflow_activity_entity.WorkflowActivities{}, err
		}
	}
